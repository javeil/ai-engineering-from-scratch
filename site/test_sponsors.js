...
test('sponsor placement uses approved artwork and destination without a tier label', () => {
  for (const file of ['README.md', 'SPONSORS.md', 'BACKERS.md']) {
    const text = read(file);
    assert.ok(text.includes(description), file);
    assert.doesNotMatch(text, /Gold sponsor|Current Gold|SerpApi[^\n]*· Gold/i);
  ...
  const placement = readme.split('### Sponsors\n')[1].split('### Use every lesson the same way')[0];
